...
> **작성일**: 2026-08-12
> **원천**: [10_acceptance_criteria.md](./10_acceptance_criteria.md)(기준 정본) · backend/tests(계약 250건 · 도메인 315건 · 서비스 13건, 전건 663건 통과) · backend/devtools/playthrough.py(연출 상수 실측) · frontend/src(화면 표면)
...
| 판정 | 수 | 비고 |
|------|:--:|------|
| ✅ 자동 | 86 | QA가 다시 돌 필요가 없다 |
| 🔶 서버만 | 24 | 화면이 붙은 뒤 화면 몫만 확인한다 |
| 👁 수동 | 2 | 반복 표본이 필요해 자동화하지 않았다 |
| 🖥 화면 | 8 | 프론트 담당 확인 항목이다 |

86 + 24 + 2 + 8 = **120**.

**QA가 사람 손으로 돌아야 하는 것은 34건**(🔶 24 · 👁 2 · 🖥 8)이며, 그중 24건은 화면 몫만 남은 항목이라 서버 거동을 다시 확인할 필요가 없다.
...
| AC-02 | ✅ | 같은 클래스 — 정원 범위 |
| AC-03 | 👁 | 코드 6자리 형식은 도메인이 보장하나 **연속 20회 발급의 중복 부재를 확인하는 테스트가 없다** |
...
| AC-35 | 👁 | **정원 상한 10명으로 6종을 돌리는 계약 테스트가 없다.** 도메인 테스트는 10인 배정을 다룬다 |
...
| AC-53 | 🔶 | 길이 상한은 확인된다. **항목 0개 직접 전송 거절은 서버에 없다(아래 갭 3)** |
...
| AC-67 | ✅ | 같은 클래스 |
| AC-68 | 🔶 | 도메인 상수가 결선 3회를 세고 저격에서 교착 경로가 확인된다. **킹메이커 3회 소진 계약 테스트가 없다** |
...
| AC-88 | 🔶 | 도메인 상수가 재대결 3회를 센다. **3회 소진 계약 테스트가 없다** |
...
| AC-107 | 🔶 | TestResult 저장 형식이 정본과 같다. **라운드 기록 표는 화면** |
...
**이 수치를 REQ-NFR-01 통과로 적지 않는다.** 두 가지가 빠져 있다.

| 빠진 것 | 내용 |
|---------|------|
| 네트워크 구간 | 루프백 측정이라 실제 사용자와 서버 사이의 지연이 0이다. 배포 환경에서 다시 잰다 |
| 서버 내부 처리 지연 | REQ-NFR-01의 다른 축인 p95 100밀리초는 서버가 도착 시각과 송출 시각을 기록해야 나온다. 계측 코드를 넣는 별도 작업이다 |

## 기준과 구현이 어긋난 지점 — 4건

확인 과정에서 나왔다. **모두 기준 문서 쪽을 고쳐야 하는 것으로 보이나, 3번은 판단이 필요하다.**

| # | 항목 | 내용 |
|:-:|------|------|
| 1 | AC-102의 에러 코드 | 기준이 game.eliminated를 요구하나 [../10_glossary/02_error_codes.md](../10_glossary/02_error_codes.md)가 그 이름을 폐기하고 game.not_eligible로 통일했다. 구현과 소켓 정본은 not_eligible이다. **AC-102를 고친다** |
| 2 | AC-82 · AC-84의 기록 무효 | [../05_game_rules/05_timer.md](../05_game_rules/05_timer.md)가 2026-08-11에 대조 실패 처리를 기록 무효에서 **서버 관측값으로 판정**으로 정정하고 기록 상태를 4값에서 3값으로 줄였다. 두 AC만 옛 서술에 남아 있다. **두 항목을 고친다** |
| 3 | AC-53의 항목 0개 | 기준은 직접 전송 시 거절을 요구하고 [../05_game_rules/03_ladder.md](../05_game_rules/03_ladder.md)는 설정 화면 검사로 규정한다. 서버는 항목 개수를 막지 않는다. **서버가 막을지 기준을 화면 검사로 좁힐지 판단이 필요하다** |
| 4 | AC-25의 에러 코드 | 기준은 game.round_already_ended를 요구하고 구현은 단계 이탈에 game.stale_phase를 쓴다. 두 코드 모두 사전에 있어 **적용 지점이 다른 것인지 확인이 필요하다** |
| 5 | 눈치 잔류 0명의 결과 표현 | 전원이 각자 혼자 눌러 잔류가 비면 최후 1인이 판정에는 LAST로 적히면서 같은 payload의 안전 확정자 목록에도 남는다. 화면이 모순된 값을 받는다. [../07_api/03_socket_events.md](../07_api/03_socket_events.md)가 이 경우를 정의하지 않는다. **판정 모듈 소관이며 규칙 확정이 선행한다** |
...
| 항목 | 내용 |
|------|------|
| 👁 2건 | AC-03(코드 20회 발급) · AC-35(정원 10명 6종)는 계약 테스트로 옮길 수 있다 |
| 🔶 중 서버 몫 | AC-68 · AC-88의 결선·재대결 3회 소진은 저격에만 계약 테스트가 있다. 킹메이커·시간초에도 같은 모양을 둔다 |
