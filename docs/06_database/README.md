...
구현 상태는 **전 테이블 ✅**다(2026-08-11). db_migration/sql이 6테이블과 제약을 만들고 verify.sql이 CHECK 24 · UNIQUE 15 · FK 9(전부 CASCADE) · VIRTUAL 4를 확인한다. backend/app/infra/db가 엔진·세션을 배선하고 서비스 계층이 트랜잭션 경계를 잡는다.
