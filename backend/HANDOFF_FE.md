...
### ③ `GameProgress.optionVotes` (킹메이커 실시간 득표) — **투표 중에는 줄 수 없다. 개표 순간은 준다**

투표가 진행되는 동안의 득표는 전역 불변식이 명시적으로 막는다 — *"진행 중에는 완료/대기 상태만 내려간다. **방장에게도 예외를 두지 않는다**"*(`docs/02_features/README.md`). `REQ-GLB-14`·`REQ-PLAY-08`이 **P0**이고 `AC-27`이 방장 화면 페이로드까지 검증 대상으로 삼는다. 중간 득표가 보이면 뒤에 투표하는 사람이 앞선 결과를 보고 고르게 되어 익명 투표의 전제가 무너진다.

**대신 개표 단계에 값을 새로 실었다.** `TALLY` 전이의 `game:phase`가 `payload`로 개표 결과를 싣는다(2026-08-12 추가).
...
> **경위 (2026-08-12)** — 실시간 공개로 한 번 결정돼 D-09를 개정하고 구현까지 마쳤다가, 화면이 **안건과 진행률**로 가기로 해 되돌렸다(커밋 9250477 → 56f9196). 그 둘은 서버가 원래부터 보내고 있어 화면 수정이 가장 적다는 판단이었다.
>
> 다시 논의된다면 **투표자 익명이 깨진다**는 점을 함께 본다. 서버가 투표자를 싣지 않아도 표가 늘어난 시점과 완료 수가 늘어난 시점이 같아 역추론된다 — 3인 방에서 한 명씩 넣으면 선택이 그대로 드러난다. `11_fairness/02_anonymity.md`가 이 경로를 이미 경고하고 있다.

#### 킹메이커 득표를 화면에 그리는 세 가지 — 전부 지금 서버로 된다

| # | 화면 | 쓰는 이벤트 | payload | 시점 |
|:-:|------|------------|---------|------|
| 1 | 진행률 — "3/5명 투표 완료" | `game:progress` | `votedCount` · `totalCount` | 표가 도착할 때마다 |
...
- 1번에는 후보별 수치가 **없다.** 넣어 달라는 요청은 위 ③의 사유로 거절됐다
