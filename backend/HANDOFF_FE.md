...
### ③ `GameProgress.optionVotes` (킹메이커 실시간 득표) — **준다** (2026-08-12 기획 결정)

요청대로 **D-09를 개정해 킹메이커만 예외로 뺐다.** `game:progress`가 후보별 득표 수를 실시간으로 싣는다.

```json
{ "payload": {
    "votedCount": 1, "totalCount": 3,
    "optionVotes": [{"optionId": "opt_...", "votes": 1}, {"optionId": "opt_...", "votes": 0}] } }
```

- **후보 전체를 매번 싣는다.** 0표 후보도 `votes: 0`으로 들어간다 — 빠지면 표가 처음 붙는 순간에만 나타나 목록이 흔들린다
- 순서는 같은 단계의 `game:phase`가 내려보낸 `candidates`와 같다(섞인 순서)
- **결선(RUNOFF)에서도 같다.** 본선과 규칙이 다르면 화면이 두 벌이 된다
- 제출 단계(SUBMIT)는 그대로 `submittedCount`만 — 안건 본문은 계속 비공개다
- **저격·시간초·눈치는 바뀌지 않았다.** D-09가 그대로 적용된다

⚠️ **투표자 익명이 이 게임에서 깨진다.** 서버는 투표자를 싣지 않지만, 3인 방에서 한 명씩 넣으면 `votedCount`가 1 오르는 순간 어느 후보가 1 오르는지 보여 선택이 드러난다. 기획 결정으로 감수한 대가이며 `11_fairness/02_anonymity.md`에 한계로 명시했다. 익명이 필요한 판은 익명 저격을 쓴다.

**개표 단계에도 값이 따로 있다.** `TALLY` 전이의 `game:phase`가 `payload`로 최종 개표 결과를 싣는다.

```json
{ "phase": "TALLY", "payload": {
    "rows": [{"candidateId": "opt_...", "text": "우리 팀 이름", "votes": 2}],
    "winnerCandidateId": "opt_..." } }
```

- **투표가 이미 마감된 뒤라 중간 집계 비공개에 걸리지 않는다.** 그 규칙이 막는 것은 표를 더 받는 동안 집계를 보여주는 것이다
- 룰렛 `SPINNING`(`winnerIndex`)·사다리 `DRAWING`(`assignments`)과 같은 자리다. `game:result`는 `TALLY` 3초가 **끝난 뒤** 오므로 그때 받으면 연출을 시작할 자리가 없다
- `rows`는 `game:result`의 `rows`와 **같은 값**이다. 계약 테스트가 두 값의 일치를 확인한다
- 결선(`RUNOFF`) 전이에는 싣지 않는다 — 표를 다시 받는 단계다

**투표 중 실시간 득표를 정말 원한다면 기획 결정이 필요하다.** P0 요구사항 2건·전역 불변식 2곳·AC-27·API 정본·공정성 문서·계약 테스트가 함께 바뀐다.

#### 킹메이커 득표를 화면에 그리는 세 가지 — 전부 지금 서버로 된다

| # | 화면 | 쓰는 이벤트 | payload | 시점 |
|:-:|------|------------|---------|------|
| 1 | 진행률 + **실시간 득표** | `game:progress` | `votedCount` · `totalCount` · `optionVotes[{optionId, votes}]` | 표가 도착할 때마다 |
| 2 | 개표 연출 — 막대가 차오르는 카운트업 | `game:phase` (`TALLY`) | `rows[{candidateId, text, votes}]` · `winnerCandidateId` | `TALLY` 진입 1회 |
| 3 | 최종 개표표 | `game:result` | `result.rows` · `winnerCandidateId` · `reveal` · `stats` | `RESULT` 진입 1회 |

**셋은 같은 판의 서로 다른 시점이라 함께 쓴다.** 투표 중에는 1번만 그리고, `TALLY` 3초 동안 2번으로 연출하고, 결과 화면에서 3번으로 확정 표를 보인다.

- 2번과 3번의 `rows`는 같은 값이며 순서도 같다(득표 내림차순). 계약 테스트가 일치를 확인하므로 **2번으로 그린 화면을 3번에서 다시 그리지 않아도 된다**
- 1번의 `optionVotes`는 **킹메이커에만** 있다. 저격·시간초·눈치의 `game:progress`에는 수치만 담긴다
- `TALLY` 길이는 서버 상수 3초다. 카운트업 길이를 그 안에 맞춘다
